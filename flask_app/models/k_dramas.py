...
class ChineseAnime:
    ...
    def get_all(cls):
        query = """
        SELECT * FROM korean_dramas
        JOIN users
        ON korean_dramas.user_id = users.id;"""
        KDrama_data = connectToMySQL(cls.db).query_db(query)
        output = []
        for user_info in KDrama_data:
    ...
    def create(cls, data ):
        query = """
        INSERT INTO korean_dramas ( title , description , date_posted ,want_to_watch,not_sure,wont_watch,user_id ) 
    ...
    def delete(cls,id):
        data = {"id" : id}
        query = "DELETE FROM korean_dramas WHERE id = %(id)s;"
    ...
    def get_one(cls,id):
        data = {"id": id}
        query = "SELECT * FROM korean_dramas WHERE id = %(id)s;"
        result = connectToMySQL(cls.db).query_db( query,data )
        return cls(result[0]) # a list of dictionaries
    
    @classmethod
    def KDrama_user(cls ,kd_id):
        data = {"id": kd_id }
        query = """
        SELECT * FROM korean_dramas
        JOIN users
        ON korean_dramas.user_id = users.id
        WHERE korean_dramas.id = %(id)s
        """
